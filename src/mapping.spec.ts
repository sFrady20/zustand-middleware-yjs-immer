...
describe("arrayToYArray", () => {
  ...
  it.each([
    [[[]], 0],
    [[1, [2, 3]], 1],
  ])("Creates nested YArrays from %s.", (array, nestedArrayIndex) => {
    ymap.set("array", arrayToYArray(array));

    expect(ymap.get("array").toJSON()).toEqual(array);
    expect(
      (ymap.get("array").get(nestedArrayIndex) as Y.Array<any>).toJSON()
  ...
  it("Creates YMaps nested in YArrays from objects nested in arrays.", () => {
    ...
    expect(ymap.get("array").get(0).get("foo")).toBe(1);
  });
  ...
  it("Creates YText nested in YArrays from strings nested in arrays.", () => {
    ...
    expect(ymap.get("array").get(0).toString()).toBe("hello");
  });
});

describe("yArrayToArray", () => {
  it.each([[[]], [[1]], [[1, 2, 3, 4]], [[[]]], [[1, [2, 3]]]])(
    "Converts a YArray of %s to an array",
    (array) => {
...
describe("arrayToYArray and yArrayToArray are inverses", () => {
  it.each([[[]], [[1]], [[1, 2, 3, 4]]])(
    "Converts arrays back into their original form. (#%#)",
    (array) => {
      const ydoc = new Y.Doc();
...
describe("objectToYMap", () => {
  ...
  it("Converts nested arrays into nested YArrays.", () => {
    ...
    expect((ymap.get("map") as Y.Map<any>).toJSON()).toEqual({ foo: [1, 2] });
...
describe("objectToYMap and yMapToObject are inverses", () => {
  it.each([[{}], [{ foo: 1 }], [{ foo: { bar: 2 } }]])(
    "Converts objects back into their original forms. (#%#)",
    (object) => {
      const ydoc = new Y.Doc();
      const ymap = ydoc.getMap("tmp");
...
describe("yTextToString", () => {
  it.each(["hello", "rawr", "goodbye"])(
    "Returns a string with the same content as the YText.",
    (string) => {
      const ydoc = new Y.Doc();
...
describe("stringToYText", () => {
  it.each(["hello", "goodbye", "wrong", "running out of things to type"])(
    "Returns a YText with the same content as the string",
    (string) => {
      const ydoc = new Y.Doc();
      const ymap = ydoc.getMap("tmp");
